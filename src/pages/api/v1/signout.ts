 ...
 *
 * 它不是一条凭据路径：这里既不认令牌也不签令牌，只是把请求带着 cookie 转给
 * can-api，再把它回的 `Set-Cookie` 原样传回浏览器。**清 cookie 的仍然是签发它
 * 的那个服务**，用的是它自己那份 Domain/Secure/SameSite —— 属性对不上的话浏览
 * 器会把过期的那枚和原来那枚一起留着，人看着退出了其实没有。
 *
 * cookie 的 Domain 是父域 `.ceruleanavi.net`，而这里是它下面的一台主机，所以由这
 * 个源把它清掉是浏览器允许的：登出在主站、雷达、开发者中心、考试中心是同一次
 * 登出。
 */
